...
public class CsvManager<T> {
    ...
    public List<T> parseCsvToEntity(MultipartFile file) throws IOException {
        List<Map<?, ?>> datas = readObjectsFromCsv(file.getInputStream());
        ObjectMapper mapper = new ObjectMapper();

        List<T> results = new ArrayList<>();
        datas.forEach(data -> {
            results.add(mapper.convertValue(data, this.tClass));
        });
    ...
    public Resource parseEntityToCsv(List<T> entities) throws IOException, IllegalAccessException {
        StringWriter writer = new StringWriter();

        //Write Headers
        List<Field> fields = Arrays.asList(this.tClass.getDeclaredFields());
