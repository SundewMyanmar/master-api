...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        Integer failedCount = (Integer) httpServletRequest.getSession().getAttribute(Constants.SESSION.AUTH_FAILED_COUNT);
        if(failedCount != null && failedCount >= securityProperties.getAuthFailedCount()){
            SecurityContextHolder.clearContext();
            httpServletResponse.sendError(HttpStatus.NOT_ACCEPTABLE.value(), "Blocked! Please contact to authority person.");
            return;
        }
